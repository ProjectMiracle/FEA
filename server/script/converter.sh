...
if [ "${SOURCE_FILE_SUFFIX}" = "webm" ]; then
        ...
        $cur_dir/silk/ffmpeg -i "$1" -ar 16000 -ac 1 "${1%.*}.$2" > /dev/null 2>&1
...
while [ $3 ]; do
	pidof $cur_dir/silk/ffmpeg&&echo -e "${RED}[Error]${RESET} ffmpeg is occupied by another application, please check it."&&exit
	...
	ls $1 | while read line; do
		...
		if [ ! -f "$2/$line.pcm" ]; then
			$cur_dir/silk/ffmpeg -y -i "$1/$line" "$2/${line%.*}.$3" > /dev/null 2>&1 &
			ffmpeg_pid=$!
			while kill -0 "$ffmpeg_pid"; do sleep 1; done > /dev/null 2>&1
		...
		$cur_dir/silk/ffmpeg -y -f s16le -ar 24000 -ac 1 -i -f wav "$2/$line.pcm" "$2/${line%.*}.$3" > /dev/null 2>&1 &
		ffmpeg_pid=$!
		while kill -0 "$ffmpeg_pid"; do sleep 1; done > /dev/null 2>&1
		rm "$2/$line.pcm"
...
$cur_dir/silk/ffmpeg -y -f s16le -ar 24000 -ac 1 -i "$1.pcm" "${1%.*}.$2" > /dev/null 2>&1 &
ffmpeg_pid=$!
while kill -0 "$ffmpeg_pid"; do sleep 1; done > /dev/null 2>&1
rm "$1.pcm"
[ ! -f "${1%.*}.$2" ]&&echo -e "${YELLOW}[Warning]${RESET} Convert $1 false, maybe ffmpeg no format handler for $2."&&exit
