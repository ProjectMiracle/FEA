...
let mandarin = multer({ dest: path.resolve('/data/release/node-weapp-demo/voice-file')});

/**
 * 上传录音文件
 */
router.post('/', mandarin.single('file'), async function (ctx, next) {
    ...
    if (res.result) {
        ...
        let recogniz = await speech.recognizeZh(base64Data.msg, fileInfo.msg.size);
        ...
        if (decodeJson == null || decodeJson === undefined) {
            decodeJson = {
                "oper": 0,
                "args": []
            };
        ...
        switch (decodeJson.oper) {
            ...
            case 2:
                ...
                console.log('exchArray: ' + (exchArray instanceof Array));
                resText = '以下是您最近的交易记录。';
